...
export default { async fetch(request, env) { const u=new URL(request.url); if(request.method==='OPTIONS') return new Response(null,{status:204,headers:cors}); try {
  ...
  if(u.pathname.startsWith('/api/auth/users/') && request.method==='PUT') return updateUser(request,env,u.pathname.split('/').pop());
  if(u.pathname.startsWith('/api/auth/users/') && request.method==='DELETE') return deleteUser(request,env,u.pathname.split('/').pop());
...
async function login(req,env){
  try {
    ...
    return out({ok:true,user:safe(user)},200,{'Set-Cookie':`amaris_session=${raw}; HttpOnly; Secure; SameSite=None; Path=/; Max-Age=${SESSION_DAYS*86400}`});
...
async function logout(req,env){const raw=cookie(req,'amaris_session');if(raw)await env.amaris_catering_db.prepare('DELETE FROM auth_sessions WHERE token_hash=?').bind(await hash(raw)).run();return out({ok:true},200,{'Set-Cookie':'amaris_session=; HttpOnly; Secure; SameSite=None; Path=/; Max-Age=0'});}
...
async function addUser(req,env){const s=await auth(req,env);if(!s||s.user.role!=='admin')return out({error:'Forbidden'},403);const b=await req.json();const username=String(b.username||'').trim().toLowerCase(),password=String(b.password||'');if(!/^[a-z0-9._-]+$/.test(username)||password.length<8)return out({error:'Username tidak valid atau password minimal 8 karakter.'},400);const id=crypto.randomUUID();try{await env.amaris_catering_db.prepare('INSERT INTO auth_users(id,username,password_hash,role,name,uptd_id,created_at) VALUES(?,?,?,?,?,?,?)').bind(id,username,await derive(password),b.role==='admin'?'admin':'uptd',String(b.name||username),String(b.uptdId||''),new Date().toISOString()).run();return out({ok:true,user:{id,username,role:b.role==='admin'?'admin':'uptd',name:String(b.name||username),uptdId:String(b.uptdId||'')}},201);}catch(e){return out({error:'Username sudah dipakai.'},409);}}
async function updateUser(req,env,id){const s=await auth(req,env);if(!s||s.user.role!=='admin')return out({error:'Forbidden'},403);const b=await req.json();const username=String(b.username||'').trim().toLowerCase();if(!/^[a-z0-9._-]+$/.test(username))return out({error:'Username tidak valid.'},400);const user=await env.amaris_catering_db.prepare('SELECT * FROM auth_users WHERE id=?').bind(id).first();if(!user)return out({error:'Akun tidak ditemukan.'},404);try{if(String(b.password||'')){await env.amaris_catering_db.prepare('UPDATE auth_users SET username=?,name=?,uptd_id=?,password_hash=? WHERE id=?').bind(username,String(b.name||username),String(b.uptdId||''),await derive(String(b.password)),id).run();}else{await env.amaris_catering_db.prepare('UPDATE auth_users SET username=?,name=?,uptd_id=? WHERE id=?').bind(username,String(b.name||username),String(b.uptdId||''),id).run();}return out({ok:true},200);}catch(e){return out({error:'Username sudah dipakai.'},409);}}
async function deleteUser(req,env,id){const s=await auth(req,env);if(!s||s.user.role!=='admin')return out({error:'Forbidden'},403);const user=await env.amaris_catering_db.prepare('SELECT role FROM auth_users WHERE id=?').bind(id).first();if(!user)return out({error:'Akun tidak ditemukan.'},404);if(user.role==='admin')return out({error:'Akun admin tidak dapat dihapus.'},400);await env.amaris_catering_db.prepare('DELETE FROM auth_users WHERE id=?').bind(id).run();return out({ok:true},200);}
